...
# Meta plots based on genes from deconv +single cell smc1
######################################################
...
#res_genes<-res[res$gene_name %in% c("CALM1","CALM2","PRKG1", "PDE5A",  "PLN", "GUCY1A1", "GUCY1B1", "LMOD1", "JUN","IGFBP5"),]
res_genes<-res_deconBulk[res_deconBulk$gene_name %in% genes_Myometrialpathway,]
...
p <- res_genes %>%
  ggplot(aes(x=gene_name, y = log2FoldChange, color=celltype)) +
  geom_errorbar(aes(ymax = log2FoldChange + 1.96*lfcSE, ymin = log2FoldChange - 1.96*lfcSE), size=1.0, width=0, position=position_dodge(width=0.7)) +
...
res_genes<-res[res$gene_name %in% genes_Myometrialpathway,]
...
# res_genes<-ref_data[ref_data$R.gene_name %in% genes_Myometrialpathway,]
...
conditions<-unlist(strsplit(colnames(bulk_matrix),"_"))[seq(2,2*length(colnames(bulk_matrix)),by=2)]  
samples<- colnames(bulk_matrix)
cvt<-data.frame(Indiv=samples, Group=conditions)


dds <- DESeqDataSetFromMatrix(round(bulk_matrix),cvt, ~ Group)    
...
res_genes<-res[res$gene_name %in% genes_Myometrialpathway,]
res_genes<-res_genes %>% filter(!is.na(padj))

p <- res_genes %>%
  ggplot(aes(x=gene_name, y = log2FoldChange)) +
  geom_errorbar(aes(ymax = log2FoldChange + 1.96*lfcSE, ymin = log2FoldChange - 1.96*lfcSE), size=1.0, width=0, position=position_dodge(width=0.7)) +
  geom_point(aes(size=padj), position=position_dodge(width=0.7)) +
  ##scale_color_manual(guide = guide_legend(reverse = TRUE) ) +
  scale_size("q-values", trans="log10", range=c(7, 1),limits=c(1E-10,1), breaks=c(1E-12,1E-6,0.001,0.01,0.1)) +
  geom_hline(yintercept=0,lty=2) + 
...
p<-res_genes %>% 
  ggplot(aes(x=reorder(gene_name,log2FoldChange),y=log2FoldChange),color="black") +
  geom_errorbar(aes(ymax = log2FoldChange + 1.96*lfcSE, ymin = log2FoldChange - 1.96*lfcSE),size=1.2,color="black",alpha=1,width=0,position=position_dodge(width=0.2)) +
  geom_point(aes(size=padj),position=position_dodge(width=0.2),color="#111111") +
  ##scale_color_manual(guide = guide_legend(reverse = TRUE) ) +
  scale_size("q-values", trans="log10", range=c(7, 1),limits=c(1E-10,1), breaks=c(1E-12,1E-6,0.001,0.01,0.1)) +
