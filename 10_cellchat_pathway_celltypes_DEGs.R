...
# # mouse colors 
# cell.type.annotation<-read.delim("../../mouse_pilot/mouse_pilot_analysis/cell-type.annotation.txt")
# clust2Names_mouse<-cell.type.annotation$Cell.type
# clust2Names_mouse<-paste0(cell.type.annotation$Cluster,"_",clust2Names_mouse)
# names(clust2Names_mouse)<-cell.type.annotation$Cluster
# 
# clust2Names_mouse<-cell.type.annotation$Cell.type
# clust2Names_mouse<-paste0(c(0:30),"_",clust2Name)
# names(clust2Names_mouse)<-c(0:30)
# 
# cluster.Colors_mouse<-c("#DF7D99","#838EDF","#4E65A6","#FFC000","#2BA3D3","#9ABF5C","#D14357","#329B2D",
...
# names(cluster.Colors_mouse)<-clust2Names_mouse #c("Stromal-1","Macrophage-2","Macrophage-1","Endothelial-1","Monocyte","CD4_T-cell","Decidual","CD8_T-cell","LED","Stromal-2","ILC","NK-cell","Smooth muscle cells-1","Myofibroblast","Macrophage-3","Endothelial-2","DC","Smooth muscle cells-2","EVT","Plasmablast","Smooth muscle cells-3","Macrophage-4","B-cell","Unciliated Epithelial")
...
# 
# 
# cluster.Colors_mouse<-cluster.Colors_mouse[order(names(cluster.Colors_mouse))]
# t2<-as.data.frame(cluster.Colors_mouse)
# t2$cluster_mouse<-rownames(t2)
# 
# cluster.Colors<-cluster.Colors[order(names(cluster.Colors))]
# t1<-cluster.Colors
# t1<-c(t1,rep(" ",nrow(t2)-length(t1)))
# t1<-as.data.frame(t1)
# t1$cluster_human<-c(names(cluster.Colors),rep(" ",nrow(t2)-length(cluster.Colors)))
# 
# rownames(t1)<-NULL
# rownames(t2)<-NULL
# 
# data.colors<-cbind(t1,t2)
# colnames(data.colors)[1]<-"cluster.Colors_human"
# data.colors<-data.colors[,c(2,1,4,3)]
# write.csv(data.colors,paste0(outFolder,"data.colors.mouse.human.csv"))

# pathways to focus 
# ignore this (for mouse project)
# selected_pathways_mat<-read.delim("selectedpathways.txt")



# adapted human colors
...
sapply (conditions ,function(xcondition){
  ...
  cellchat<-read_rds(paste0("./10_CellChat_analysis_default_after_filter_",threshold,"/","cellchat_",xcondition,"_","2022-02-11",".rds"))
  ...
  sapply(selected_pathways,function(pathways.show){
    ...
    system(paste0("mkdir -p ", outFolder,xcondition,filename))
    pdf(paste0(outFolder,xcondition,filename,pathways.show,".pdf"),width=12,height=12)
    par(mfrow=c(1,1))
    ...
    pdf(paste0(outFolder,xcondition,filename,pathways.show,"_nolabel.pdf"),width=12,height=12)
    par(mfrow=c(1,1))
...
#for (xcondition in conditions){
  ...
  #
    ...
    selected_pathways<-pathways #selected_pathways[selected_pathways!=""]
    system(paste0("mkdir -p ", outFolder,xcondition,"/"))
  ...
  fname=paste0(outFolder,xcondition,"/",xcondition,"_outgoing_ggalluvial_cuttoff_",Sys.Date(),"_.pdf");
  
  pdf(fname,width=20,height=30)
  ...
  fname=paste0(outFolder,xcondition,"/",xcondition,"_incoming_ggalluvial_cuttoff_",Sys.Date(),"_.pdf")
  pdf(fname,width=20,height=30)
...
# clust2Names<-cell.type.annotation$Cell.type
# clust2Names<-paste0(c(0:30),"_",clust2Name)
# names(clust2Names)<-c(0:30)
