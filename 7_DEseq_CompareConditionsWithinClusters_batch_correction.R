...
# sc$Cell_type<-clust2Names[sc$seurat_clusters]
...
resList <- lapply(1:nrow(kSize),function(ii){
                      ...
                      # Library<-sapply(colnames(cmat), function(x)
                      # {
                      #   
                      # y<-md %>% filter (GroupPregnancy_ID ==x & cname1==cname ) %>% dplyr::select (Library)%>% unlist() 
                      # return(unique(y ))
                      #     
                      # })
